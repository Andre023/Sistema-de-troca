...
export default function ExchangeControl() {
    const { suppliers: suppliersPagination = {}, filters = { sort: 'name', direction: 'asc' }, flash, errors, auth } = usePage().props;
    const suppliers = suppliersPagination.data ?? [];

    const isAdmin = auth?.user?.role === 'admin';
    const [newBoxObservation, setNewBoxObservation] = useState('');
    ...
    const handleSort = (column) => {
        ...
        router.get(
            ...
            { per_page: perPage, sort: column, direction: newDirection },
            { preserveState: true, preserveScroll: true }
        );
    };

    const updateBoxObservation = (boxId, observation) => {
        if (!isAdmin) return;
        router.put(route('exchange-control.boxes.update', boxId), { observation }, { preserveScroll: true });
    };

    // Substitua o submitBox existente por este:
    const submitBox = (event) => {
        ...
        const payload = { number: boxNumber };
        if (isAdmin) payload.observation = newBoxObservation;

        router.post(
            route('exchange-control.boxes.store', selectedSupplier.id),
            payload,
            { preserveScroll: true, onSuccess: () => { setNewBoxNumber(''); setNewBoxObservation(''); } }
        );
    };
    ...
    const updateSupplierObservation = (supplierId, observation) => {
        if (!isAdmin) return;
        router.put(route('exchange-control.suppliers.updateObservation', supplierId), { observation }, { preserveScroll: true });
    };
    ...
    const filterBySupplier = (supplierId) => {
        // Esconde os resultados da busca e filtra a tabela
        setHasSearched(false);
        setSearchTerm('');
        router.get(
            route('exchange-control.index'),
            { supplier_id: supplierId, per_page: perPage, sort: filters.sort, direction: filters.direction },
            { preserveState: true, preserveScroll: true }
        );
    };
    ...
    const clearFilter = () => {
        router.get(
            route('exchange-control.index'),
            { per_page: perPage, sort: filters.sort, direction: filters.direction },
            { preserveState: true, preserveScroll: true }
        );
    };
            ...
            <div className="mx-auto max-w-7xl space-y-6 px-4 py-12 sm:px-6 lg:px-8">
                <div className="rounded-xl border border-indigo-100 bg-gradient-to-r from-indigo-50 via-white to-blue-50 p-4 shadow-sm sm:p-6">
                    <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
                        ...
                        {/* Procure por este bloco e adicione o !isAdmin && */}
                        {!isAdmin && (
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {searchResult.boxMatches.map((match, idx) => (
                                            <div key={`box-match-${idx}`} className="flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-lg bg-white p-3 shadow-sm text-sm border border-emerald-100 gap-3">
                                                ...
                                                <button type="button" onClick={() => filterBySupplier(match.supplier.id)} className="w-full sm:w-auto rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white transition hover:bg-emerald-700">
                                                    Ir para fornecedor
                                                </button>
                                            </div>
                                        ))}
                            ...
                            {searchResult.eanMatch ? (
                                <div className="mb-2 rounded-lg border border-indigo-100 bg-indigo-50/70 p-4 text-sm">
                                    ...
                                    {searchResult.eanMatch.linked_supplier ? (
                                        <button type="button" onClick={() => filterBySupplier(searchResult.eanMatch.linked_supplier.id)} className="mt-3 inline-block rounded-md bg-indigo-100 px-3 py-1 text-indigo-800 font-semibold text-xs hover:bg-indigo-200 transition">
                                            Ver este fornecedor na tabela
                                        </button>
                                    ) : (
                            ...
                            {searchResult.suppliers?.length ? (
                                <div className="flex flex-wrap gap-2 mt-3">
                                    {searchResult.suppliers.map((supplier) => (
                                        <button key={`search-${supplier.id}`} type="button" onClick={() => filterBySupplier(supplier.id)} className="rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-sm font-medium text-indigo-700 transition hover:bg-indigo-100">
                                            {supplier.name}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="hidden overflow-x-auto md:block">
                                ...
                                {filters.supplier_id && (
                                    <div className="mb-4 flex items-center justify-between rounded-lg bg-indigo-50 p-3 border border-indigo-100">
                                        <span className="text-sm font-semibold text-indigo-800">Mostrando apenas o fornecedor pesquisado.</span>
                                        <button type="button" onClick={clearFilter} className="rounded bg-indigo-600 px-3 py-1.5 text-xs font-bold text-white transition hover:bg-indigo-700">Limpar filtro</button>
                                    </div>
                                )}
                                ...
                                <thead>
                                    <tr className="border-b text-left text-sm uppercase tracking-wide text-gray-500">
                                        <th 
                                            ...
                                            title="Clique para ordenar de A a Z"
                                        >
                                            Fornecedor {filters.sort === 'name' ? (filters.direction === 'asc' ? '↑' : '↓') : ''}
                                        </th>
                                        <th 
                                            className="px-4 py-3 cursor-pointer select-none hover:bg-gray-100 transition"
                                            onClick={() => handleSort('boxes_count')}
                                        >
                                            Qtd. Caixas {filters.sort === 'boxes_count' ? (filters.direction === 'asc' ? '↑' : '↓') : ''}
                                        </th>
                                    ...
                                    {suppliers.map((supplier) => (
                                        <Fragment key={supplier.id}>
                                            ...
                                            >
                                                ...
                                                {/* Adicionado stopPropagation aqui para não abrir/fechar a gaveta ao clicar no input */}
                                                <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                                                    {isAdmin ? (
                                                        <input
                                                            type="text"
                                                            defaultValue={supplier.observation || ''}
                                                            placeholder="Adicionar observação..."
                                                            onBlur={(e) => updateSupplierObservation(supplier.id, e.target.value)}
                                                            className="w-full rounded border-gray-300 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                                                        />
                                                    ) : (
                                                        <span className="text-sm text-gray-600">{supplier.observation || '-'}</span>
                                                    )}
                                                </td>
                                                {!isAdmin && (
                                                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                                                        <div className="flex justify-end gap-2">
                                                            <button type="button" onClick={() => openBoxesModal(supplier)} className="rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-emerald-700 transition hover:bg-emerald-100" title="Gerenciar caixas">
                                                                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 5v14M5 12h14" /></svg>
                                                            </button>
                                                            <button type="button" onClick={() => openEditModal(supplier)} className="rounded-lg border border-blue-200 bg-blue-50 p-2 text-blue-700 transition hover:bg-blue-100" title="Editar">
                                                            ...
                                                            <button type="button" onClick={() => removeSupplier(supplier)} className="rounded-lg border border-rose-200 bg-rose-50 p-2 text-rose-700 transition hover:bg-rose-100" title="Excluir">
                                            ...
                                            {/* GAVETA COM AS CAIXAS (De volta ao original) */}
                                            <tr className="border-b bg-gray-50/60">
                                                <td colSpan={isAdmin ? 3 : 4} className="p-0">
                    ...
                    <div className="fixed inset-0 overflow-y-auto"><div className="flex min-h-full items-center justify-center p-4">
                        <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <Dialog.Panel className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl">
                                ...
                                <form onSubmit={submitBox} className="mt-4 flex flex-col gap-2">
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        {isAdmin && (
                                            <input type="text" value={newBoxObservation} onChange={(e) => setNewBoxObservation(e.target.value)} placeholder="Observação (opcional)" className="flex-1 rounded-lg border border-gray-300 p-2.5 focus:border-emerald-500" />
                                        )}
                                ...
                                <div className="mt-5 flex max-h-72 flex-col gap-3 overflow-y-auto rounded-lg border border-gray-200 p-3">
                                    {selectedSupplier?.boxes?.length ? selectedSupplier.boxes.map((box) => (
                                        <div key={box.id} className="flex flex-col gap-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm">
                                            <div className="flex items-center justify-between">
                                                <span className="font-semibold text-gray-800">Caixa {box.number}</span>
                                                <button type="button" onClick={() => removeBox(box.id)} className="font-semibold text-rose-600 transition hover:text-rose-700">Remover</button>
                                            </div>
                                            {isAdmin ? (
                                                <input
                                                    type="text"
                                                    defaultValue={box.observation || ''}
                                                    placeholder="Adicionar observação..."
                                                    onBlur={(e) => updateBoxObservation(box.id, e.target.value)}
                                                    className="w-full text-xs rounded border border-gray-300 p-1.5 mt-1 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                                                />
                                            ) : (
                                                box.observation && <span className="text-xs text-gray-600 italic">Obs: {box.observation}</span>
                                            )}
                                        </div>
